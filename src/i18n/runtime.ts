...
const CATALOG:
  ...
  /*
   * Arc testnet trial. Added 2026-08-31.
   */
  ...
  [
    'Arc üzerinde USDC ile öde, 3 gün hediye al',
    'Pay in USDC on Arc, get 3 days free',
    'Оплати в USDC на Arc — получи 3 дня в подарок',
  ],

  [
    'Arc, Circle’ın stablecoin için tasarladığı yeni bir blokzincir — gaz ücreti bile USDC ile ödeniyor. Cloud Miner aboneliğini Arc test ağı üzerinden denedik ve akışı sana açtık: birkaç dakikada tamamlanıyor, sana hiçbir maliyeti yok.',
    'Arc is Circle’s new stablecoin-native blockchain — even gas is paid in USDC. We ran the Cloud Miner subscription through Arc’s test network and opened the flow to you: it takes a few minutes and costs you nothing.',
    'Arc — новый блокчейн Circle для стейблкоинов: даже комиссия платится в USDC. Мы провели подписку Cloud Miner через тестовую сеть Arc и открыли этот путь для вас: пара минут и никаких затрат.',
  ],
  ...
  [
    'MetaMask yeterli. Arc Testnet ağını elle eklemene gerek yok — ödeme sayfası cüzdanına ağı kendisi tanıtıyor, sen onaylıyorsun.',
    'MetaMask is enough. You do not need to add the Arc Testnet network by hand — the payment page introduces it to your wallet and you approve.',
    'Достаточно MetaMask. Сеть Arc Testnet вручную добавлять не нужно — страница оплаты сама предложит её, вы подтверждаете.',
  ],

  [
    'Ücretsiz test USDC al',
    'Get free test USDC',
    'Получи бесплатные тестовые USDC',
  ],
  ...
  [
    'Yukarıdaki karttan denemeyi başlat, çıkan ödeme sayfasında 1 test USDC gönder. Ödeme zincirde onaylandığı an aboneliğine 3 gün ekleniyor — ortalama yarım dakika sürüyor.',
    'Start the trial from the card above and send 1 test USDC on the payment page that opens. The moment the payment is confirmed on chain, 3 days are added to your subscription — it takes about half a minute.',
    'Начни пробный период на карточке выше и отправь 1 тестовый USDC на открывшейся странице оплаты. Как только платёж подтвердится в сети, к подписке добавятся 3 дня — обычно за полминуты.',
  ],
  ...
  [
    'Bu bir test ağı denemesidir, satın alma değil. Kullanılan USDC gerçek para değildir ve herhangi bir bedeli yoktur. Acki Nacki Radar bağımsız bir projedir; Circle veya Arc ekibiyle bir ortaklığı ya da onayı yoktur.',
    'This is a test network trial, not a purchase. The USDC used is not real money and has no cost. Acki Nacki Radar is an independent project with no partnership with or endorsement from Circle or the Arc team.',
    'Это пробный период в тестовой сети, а не покупка. Используемые USDC не являются настоящими деньгами. Acki Nacki Radar — независимый проект без партнёрства с Circle или командой Arc.',
  ],
  ...
  [
    'Arc’ı dene, hediye kazan',
    'Try Arc, earn free days',
    'Попробуй Arc и получи подарок',
  ],
  ...
  [
    ...
    'A test network trial, not a purchase. The USDC you will use comes free from the Circle faucet and is not real money.',
    'Это пробный период в тестовой сети, а не покупка. USDC выдаёт бесплатный кран Circle, это не настоящие деньги.',
  ],
  ...
  [
    ...
    'Your subscription opens the moment the payment is confirmed on chain. You can close this page.',
    'Подписка активируется, как только платёж подтвердится в сети. Эту страницу можно закрыть.',
  ],

  [
    'Arc hediyesi hesabına tanımlandı',
    'Your Arc reward has been applied',
    'Подарок Arc зачислен на аккаунт',
  ],
  ...
  [
    'Bu hesap Arc denemesini kullandı. Deneme hesap başına bir kezdir; aboneliğini uzatmak için Planlar sayfasını kullanabilirsin.',
    'This account has used its Arc trial. The trial is once per account; use the Plans page to extend your subscription.',
    'Этот аккаунт уже использовал пробный период Arc. Он даётся один раз; продлить подписку можно на странице «Планы».',
  ],
  ...
  [
    ...
    'The quota is full. We will announce it here when we open more.',
    'Квота исчерпана. Мы сообщим здесь, когда откроем новую.',
  ],

  [
    'Deneme şu anda kapalı.',
    'The trial is currently closed.',
    'Пробный период сейчас закрыт.',
  ],
