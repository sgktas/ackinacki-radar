...
type MiningWatchRecord = {
  ...
  events: MiningEvent[];
  dailyTotals?: MiningDailyTotal[];
};

type MiningMonitorState = {
  watches: MiningWatchRecord[];
  nextScanCursor?: number;
  nextActiveScanCursor?: number;
  nextPassiveScanCursor?: number;
...
function readPaymentsState(): PaymentsState {
  ...
  if (!raw.trim()) {
    return { lastCheckedBalanceRaw: null, pendingInvoices: [], subscriptions: {} };
  }
...
const BEE_MINING_SESSION_DURATION_MS =
  Number(process.env.BEE_MINING_SESSION_SECONDS || 240) * 1000; // leaves buffer before next epoch
...
// Reference miner uses "1/3 in 1s". Our sessions are 120s inside a 300s tick,
// so only ~1 extra full session actually fits; the loop also checks the clock
// and stops early rather than overrunning the slot. Env-tunable.
const BEE_CONGESTION_RETRIES = Number(process.env.BEE_CONGESTION_RETRIES || 3);
...
const beeSessionInFlight = new Map<string, boolean>();
...
async function runBeeMiningTick() {
  ...
  try {
    ...
    const activeMiners = state.miners.filter((miner) => miner.status === "active");
    ...
    const worker = async () => {
      while (index < activeMiners.length) {
        ...
        try {
          ...
          if (beeSessionInFlight.get(minerAddress)) {
          ...
          beeSessionInFlight.set(minerAddress, true);
...
function startBeeMiningScheduler() {
  ...
  const scheduleAt = (targetMs: number) => {
    ...
    beeMiningTimer = setTimeout(() => {
      void runBeeMiningTick().finally(() => {
        const nextTarget = targetMs + BEE_MINING_INTERVAL_MS;
        scheduleAt(Math.max(nextTarget, Date.now() + MIN_RESCHEDULE_FLOOR_MS));
      });
    }, delayMs);
  };

  console.log("Bee mining scheduler started:", {
    intervalSeconds: Math.round(BEE_MINING_INTERVAL_MS / 1000),
    sessionSeconds: Math.round(BEE_MINING_SESSION_DURATION_MS / 1000),
    tapCount: BEE_MINING_TAP_COUNT,
    concurrency: BEE_MINING_CONCURRENCY,
  });

  scheduleAt(Date.now() + BEE_MINING_INITIAL_DELAY_MS);
}
...
async function runPaymentsCheckTickSenderBased(
  ...
  for (const transfer of newTransfers) {
    ...
    // Accept the invoice whose price the transfer covers (paying slightly
    // more than the plan price is fine; paying less is not a match).
    const matchedInvoice = candidateInvoices
      .sort((a, b) => BigInt(a.amountRaw) < BigInt(b.amountRaw) ? -1 : 1)
      .find((invoice) => BigInt(transfer.shellValueRaw) >= BigInt(invoice.amountRaw));

    if (!matchedInvoice) {
      console.log(
        "Payments check (sender-based): recognized wallet paid but no matching pending invoice:",
        { chatId, src: transfer.src, shell: formatShellAmount(transfer.shellValueRaw) },
      );
      continue;
    }
  ...
  bot.command("plan_buy", async (ctx) => {
    ...
    await ctx.reply(
      [
        ...
        `<code>${escapeHtml(formatShellAmount(plan.priceShellRaw))}</code> SHELL`,
        "",
